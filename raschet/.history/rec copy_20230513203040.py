...
def calc():
    ...
    Args:
        ...
        alpha (float): dark Fine-structure constant.
        ma (float): Mass of dark electron.
        mb (float): Mass of dark proton.
...
def main():
    alp_range = [10**i for i in np.arange(-1,3,0.5)]
    m_range = [10**(9+i) for i in np.arange(-6,3,0.5)]
    ...
    sns.heatmap(df, annot=True, linewidths=.5, ax=ax, cmap='coolwarm', cbar_kws={'label': 'Value'})
    ...
    ax.xaxis.set_major_formatter(ticker.FuncFormatter(funcForFormatter))
