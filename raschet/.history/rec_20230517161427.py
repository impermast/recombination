...
def heatplot(df):
    ...
    ax.set(xlabel=r'$m_a$, эВ', ylabel=r'$\alpha_y$', title='Относительная плотность прорекомбинировавших частиц для различных параметров модели')
...
def graphlim(T,rk,r3,rcl,rlim):
    # graphplotter used to plot r(T) graphs
    fig, ax = plt.subplots(1,1)
    ax.plot(T,r3,'b',label ="Трехчастичная рекомбинация")
    ax.plot(T,rlim, "--g",label ="Предел применимости")
    # ax.plot(T,r2,'g', label ="Радиационная рекомбинация")
    ax.set_xlabel('T, эВ', fontsize=14)
    ax.set_ylabel(r'$r/r_0$', fontsize=14)
    ax.set_xlim([10**7, 10**3])
    ax.set_ylim([10**(-10), 10**(8)])
    ax.text(10**(4.5), 10**(-7),
            r'$m_a = 10^{%0.0f}$ эВ'%(np.log10(ma))+"\n"+
            r'$m_b = 10^{%0.0f}$ эВ'%(np.log10(mb))+"\n"+
            r'$\alpha = $'+repr(alpha), 
    ...
    plt.semilogy()
    plt.show()
def classkram3body(T,rk,r3,rcl,rlim):
    # graphplotter used to plot r(T) graphs
    fig, ax = plt.subplots(1,1)
    ax.plot(T,r3,'b',label ="Трехчастичная рекомбинация")
    ax.plot(T,rk,'r', label ="Радиационная рекомбинация")
    ax.plot(T,rcl,'g', label ="Классическая рекомбинация")
    ax.set_xlabel('T, эВ', fontsize=14)
    ax.set_ylabel(r'$r/r_0$', fontsize=14)
    ...
    ax.text(10**(8), 10**(-11),
            ...
            r'$m_b = 10^{%0.0f}$ эВ'%(np.log10(mb))+"\n"+
            r'$\alpha = $'+repr(alpha), 
            fontsize=12, bbox={'facecolor': 'WhiteSmoke', 'alpha': 0.5, 'pad': 10})
    ax.tick_params(labelsize=12)
    ax.grid(color='gray', linestyle='--', linewidth=0.5, alpha=0.5)
    ax.legend(fontsize=12)
    ax.invert_xaxis()
